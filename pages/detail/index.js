...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.request({
      url: 'https://win-east.cn/blog/public/api/art/' + options.cid+'/'+app.globalData.user_id,
      success: function(res) {
        console.log(11, res)
        if (!res.data.art) {
          that.setData({
        ...
        that.setData({
          info: res.data.art,
          show: 1,
          source_click: res.data.art.is_like,
          is_coll: res.data.art.is_coll
        });
        wx.hideLoading()
      }
    })
  ...
  topicColl: function () {
    ...
    if (!app.globalData.user_id) {
      ...
      wx.request({
        url: 'https://win-east.cn/blog/public/api/addcoll/' + that.data.art_id+'/'+ app.globalData.user_id,
        success: function (res) {
          console.log(res)
          if (res.data.status == 10001) {
            that.setData({
              is_coll: that.data.is_coll?0:1
            })
          }
        }
      })
